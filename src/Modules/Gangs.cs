    ...
    public class Gangs : DEAModule
    {
        ...
        public async Task ResetCooldowns([Remainder] string name)
        {
            ...
            if (user.Cash < Config.GANG_CREATION_COST)
                await Error($"You do not have {Config.GANG_CREATION_COST.ToString("C", Config.CI)}. Balance: {user.Cash.ToString("C", Config.CI)}.");
        ...
        {
            if (GangRepository.InGang(user.Id, Context.Guild.Id)) await Error("This user is already in a gang.");
            if (GangRepository.IsFull(Context.User.Id, Context.Guild.Id)) await Error("Your gang is already full!");
        ...
        {
            Console.WriteLine(GangRepository.InGang(Context.User.Id, Context.Guild.Id));
            Console.WriteLine(gangName == null);
            bool inGang = GangRepository.InGang(Context.User.Id, Context.Guild.Id);
            if (gangName == null && !inGang)
                await Error($"You are not in a gang.");
            Gang gang;
        ...
        public async Task Ganglb()
        {
            ...
            if (gangs.Count == 0) await Error("There aren't any gangs yet.");
        ...
        public async Task LeaveGang()
        {
            ...
            if (gang.LeaderId == Context.User.Id)
                await Error($"You may not leave a gang if you are the owner. Either destroy the gang with the `{prefix}DestroyGang` command, or " +
        ...
        public async Task KickFromGang([Remainder] IGuildUser user)
        {
            if (user.Id == Context.User.Id)
                await Error("You may not kick yourself!");
            if (!GangRepository.IsMemberOf(Context.User.Id, Context.Guild.Id, user.Id))
                await Error("This user is not a member of your gang!");
            var gang = GangRepository.FetchGang(Context);
        ...
        public async Task ChangeGangName([Remainder] string name)
        {
            ...
            if (user.Cash < Config.GANG_NAME_CHANGE_COST)
                await Error($"You do not have {Config.GANG_NAME_CHANGE_COST.ToString("C", Config.CI)}. Balance: {user.Cash.ToString("C", Config.CI)}.");
            var gangs = DEABot.Gangs.Find(y => y.GuildId == Context.Guild.Id).ToList();
            if (gangs.Any(x => x.Name.ToLower() == name.ToLower())) await Error($"There is already a gang by the name {name}.");
        ...
        {
            if (user.Id == Context.User.Id) await Error("You are already the leader of this gang!");
            var gang = GangRepository.FetchGang(Context);
            if (!GangRepository.IsMemberOf(Context.User.Id, Context.Guild.Id, user.Id)) await Error("This user is not a member of your gang!");
        ...
        {
            var user = UserRepository.FetchUser(Context);
            if (cash < Config.MIN_DEPOSIT) await Error($"The lowest deposit is {Config.MIN_DEPOSIT.ToString("C", Config.CI)}.");
            if (user.Cash < cash) await Error($"You do not have enough money. Balance: {user.Cash.ToString("C", Config.CI)}.");
        ...
        public async Task Withdraw(decimal cash)
        {
            var gang = GangRepository.FetchGang(Context);
            var user = UserRepository.FetchUser(Context);
            if (cash < Config.MIN_WITHDRAW) await Error($"The minimum withdrawal is {Config.MIN_WITHDRAW.ToString("C", Config.CI)}.");
            if (cash > gang.Wealth * Config.WITHDRAW_CAP)
                await Error($"You may only withdraw {Config.WITHDRAW_CAP.ToString("P")} of your gang's wealth, " +
