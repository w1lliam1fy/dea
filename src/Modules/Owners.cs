...
namespace DEA.Modules
{
    ...
    public class Owners : DEAModule
    {
        ...
        {
            if (money < 0) await Error("You may not add negative money to a user's balance.");
        ...
        {
            if (money < 0) await Error("You may not remove a negative amount of money from a user's balance.");
            await UserRepository.EditCashAsync(Context, userMentioned.Id, -money);
        ...
        {
            if (permissionLevel < 1 || permissionLevel > 3) await Error("Permission levels:\nModeration: 1\nAdministration: 2\nServer Owner: 3");
        ...
        public async Task RemoveModRole(IRole modRole)
        {
            ...
            if (!guild.ModRoles.Any(x => x.Name == modRole.Id.ToString()))
                await Error("This role is not a moderator role!");
